...
class MyWindowSHAP():
    def __init__(self, model_func, test_data, background_data, stride = 5, window_len = 20 , delta = 0.3, n_w = 8, method = "stationary"):

        # reshape the data to the required format # (samples, time steps, channels)
        ws_test_data = test_data.reshape((test_data.shape[0], test_data.shape[2], test_data.shape[1]))
        ws_bg_data = background_data.reshape((background_data.shape[0], background_data.shape[2], background_data.shape[1]))
        self.model_func = model_func    
        # initialize the WindowSHAP object
        if method == 'stationary':
            self.WindowSHAP = StationaryWindowSHAP(model = self.wrap_model_func, window_len=window_len,B_ts = ws_bg_data, test_ts=ws_test_data)
        elif method == 'sliding':
            self.WindowSHAP = SlidingWindowSHAP(model = self.wrap_model_func, stride=stride, window_len=window_len,B_ts = ws_bg_data, test_ts=ws_test_data)
        elif method == 'dynamic':
            self.WindowSHAP = DynamicWindowSHAP(model = self.wrap_model_func, delta=0.3, n_w=8, B_ts = ws_bg_data, test_ts=ws_test_data)

    def wrap_model_func(self,X):
        f_output = self.model_func(X)

        if len(f_output.shape) == 1:
            return f_output.reshape((f_output.shape[0],1)) 
        else:
            return f_output[:,[0]] # only take the first column
        

    def shap_values(self):
        sv = self.WindowSHAP.shap_values()
        return sv.reshape((sv.shape[0], sv.shape[2], sv.shape[1]))
...
class StationaryWindowSHAP():
    ...
    def wraper_predict(self, x):
        ...
        if self.model_type == 'lstm_dem':
            ...
            model_input = ts_x_.reshape((ts_x_.shape[0], ts_x_.shape[2], ts_x_.shape[1]))
    ...
    def shap_values(self, num_output=1):
        ...
        shap_values = self.explainer.shap_values(self.test_data, silent=True)
...
class SlidingWindowSHAP():
    ...
    def shap_values(self, num_output=1, nsamples='auto'):
        ...
        for stride_cnt in range(num_sw):
            ...
            shap_values = self.explainer.shap_values(self.test_data, nsamples=nsamples, silent=True)
            shap_values = np.array(shap_values)
...
class DynamicWindowSHAP():
    ...
    def shap_values(self, num_output=1, nsamples_in_loop='auto', nsamples_final='auto'):
        ...
        while flag:
            ...
            for i in range(self.num_ts_ftr):
                ...
                for j in range(self.num_window[i]):                             
                    ...
                    if abs(ts_shap_values[0, sum(self.num_window[:i]) + j, 0]) > self.delta:
        ...
        self.explainer = shap.KernelExplainer(self.wraper_predict, self.background_data)
        shap_values = self.explainer.shap_values(self.test_data, nsamples=nsamples_final, silent=True)
        shap_values = np.array(shap_values)
        dem_phi = shap_values[0, :, :self.num_dem_ftr] # Extracting dem SHAP values
        ts_shap_values = shap_values[:, :, self.num_dem_ftr:] # Extracting ts SHAP values
